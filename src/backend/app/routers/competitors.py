...
class AddCompetitorRequest(BaseModel):
    name: str = Field(..., min_length=1, max_length=255, description="Display name for the competitor")
    platform: str = Field(..., description="Platform: twitter, linkedin, instagram, youtube, tiktok, facebook, blog")
    handle: str = Field(..., min_length=1, max_length=255, description="Username/handle on the platform")
    description: Optional[str] = Field(default=None, description="Optional description")
    profile_url: Optional[str] = Field(default=None, description="Optional direct profile URL")
...
async def add_competitor(
    ...
    # Validate platform
    valid_platforms = ["twitter", "linkedin", "instagram", "youtube", "tiktok", "facebook", "blog", "newsletter", "other"]
    ...
    try:
        ...
        if "unique_competitor_per_user" in str(e).lower() or "duplicate" in str(e).lower():
            raise HTTPException(
...
@router.get("/competitors/{competitor_id}/content", response_model=List[CompetitorContentResponse])
...
async def get_content_gaps(
    min_opportunity: int = Query(0, ge=0, le=100, description="Minimum opportunity score filter"),
    user=Depends(get_auth_user)
):
    """
...
@router.post("/competitors/{competitor_id}/refresh", response_model=RefreshCompetitorResponse)
...
async def get_supported_platforms():
    ...
    """
    return ["twitter", "linkedin", "instagram", "youtube", "tiktok", "facebook", "blog", "newsletter", "other"]