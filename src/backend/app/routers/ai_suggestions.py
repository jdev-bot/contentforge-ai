...
class ContentImprovementRequest(BaseModel):
    content_id: UUID
    suggestion_type: str = Field(..., description="Type: readability, engagement, clarity, grammar")
...
class ToneAdjustmentRequest(BaseModel):
    content_id: UUID
    tone: str = Field(..., description="Options: professional, casual, humorous, formal, friendly, authoritative")
...
async def get_content_improvements(
    ...
    try:
        # Get the content
        content_result = supabase.table("content").select("*").eq("id", str(request.content_id)).eq("user_id", str(user.id)).single().execute()
        ...
        system_prompt = system_prompts.get(request.suggestion_type, system_prompts["readability"])
...
async def get_seo_optimization(
    ...
    try:
        ...
        keywords_str = ", ".join(request.keywords) if request.keywords else "auto-detect"
...
2. [Another heading suggestion]"""
        ...
        try:
            ...
            if "READABILITY_SCORE:" in result:
                score_line = [l for l in result.split('\n') if 'READABILITY_SCORE:' in l][0]
                readability_score = int(''.join(filter(str.isdigit, score_line)))
            
            if "KEYWORD_DENSITY:" in result:
                kd_section = result.split("KEYWORD_DENSITY:")[1].split("SUGGESTIONS:")[0]
                for line in kd_section.split('\n'):
                    if ':' in line and '-' in line:
                        parts = line.replace('-', '').strip().split(':')
                        if len(parts) == 2:
                            keyword_density[parts[0].strip()] = float(parts[1].replace('%', '').strip())
            
            if "SUGGESTIONS:" in result:
                sug_section = result.split("SUGGESTIONS:")[1].split("META_TITLE_SUGGESTION:")[0]
                for line in sug_section.split('\n'):
                    if line.strip() and (line.strip()[0].isdigit() or line.strip().startswith('-')):
                        suggestions.append(line.replace('-', '').replace('1.', '').replace('2.', '').replace('3.', '').strip())
            
            if "META_TITLE_SUGGESTION:" in result:
                mt_section = result.split("META_TITLE_SUGGESTION:")[1].split("META_DESCRIPTION_SUGGESTION:")[0]
                meta_title = mt_section.strip()[:60]
            
            if "META_DESCRIPTION_SUGGESTION:" in result:
                md_section = result.split("META_DESCRIPTION_SUGGESTION:")[1].split("HEADING_STRUCTURE:")[0]
...
Respond with just the tone descriptor (e.g., formal, casual, professional, humorous)."""
        ...
        original_tone = original_tone.strip().lower().split()[0] if original_tone.strip() else "neutral"
...
2. Three key characteristics of this tone that were applied"""
        ...
        tone_characteristics = {
            ...
            "vocabulary_level": "elevated" if request.tone in ["formal", "professional", "authoritative"] else "conversational",
            "sentence_structure": "complex" if request.tone in ["formal", "professional"] else "varied",
            "emotional_quality": "neutral" if request.tone == "formal" else "warm" if request.tone == "friendly" else "confident",
        }
...
@router.get("/ai-suggestions/{content_id}", response_model=List[AIImprovementSuggestion])
async def list_suggestions(
    ...
    try:
        query = supabase.table("ai_suggestions").select("*").eq("content_id", str(content_id)).eq("user_id", str(user.id))
...
@router.patch("/ai-suggestions/{suggestion_id}/apply", response_model=AIImprovementSuggestion)
async def apply_suggestion(
    ...
    try:
        # Get the suggestion
        suggestion_result = supabase.table("ai_suggestions").select("*").eq("id", str(suggestion_id)).eq("user_id", str(user.id)).single().execute()
        ...
        update_result = supabase.table("ai_suggestions").update({"applied": True}).eq("id", str(suggestion_id)).execute()
        
        # Update the content with the improved text
        supabase.table("content").update({
            "original_text": suggestion["improved_text"],
            "updated_at": "now()"
...
async def list_seo_analyses(
    ...
    try:
        result = supabase.table("seo_analyses").select("*").eq("content_id", str(content_id)).eq("user_id", str(user.id)).order("created_at", desc=True).execute()
...
@router.get("/ai-suggestions/{content_id}/tone", response_model=List[ToneAdjustmentResult])
async def list_tone_adjustments(
    content_id: UUID,
    user=Depends(get_auth_user)
):
    ...
    try:
        result = supabase.table("tone_adjustments").select("*").eq("content_id", str(content_id)).eq("user_id", str(user.id)).order("created_at", desc=True).execute()
