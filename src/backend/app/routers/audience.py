...
class AudienceMetricCreate(BaseModel):
    ...
    platform: str = Field(..., description="Social media platform (twitter, linkedin, youtube, etc.)")
    metric_type: str = Field(..., description="Type of metric (followers, subscribers, engagement_rate, etc.)")
    value: int = Field(..., ge=0, description="Metric value")
    period: str = Field(default="daily", description="Time period (daily, weekly, monthly)")
    recorded_at: Optional[datetime] = Field(default=None, description="Timestamp for the metric")
...
class WebhookPayload(BaseModel):
    ...
    metadata: Optional[Dict[str, Any]] = Field(default=None, description="Additional platform data")
...
async def get_growth_metrics(
    platform: Optional[str] = Query(None, description="Filter by platform (e.g., twitter, linkedin)"),
...
async def receive_webhook(
    ...
    try:
        # Validate platform
        valid_platforms = ["twitter", "linkedin", "youtube", "instagram", "tiktok", "facebook"]
...
async def get_insights(
    ...
    try:
        insights = audience_service.generate_insights(
            user_id=str(user.id),
            days=days
        )
...
async def generate_snapshot(user=Depends(get_auth_user)):
    ...
    try:
        snapshot = audience_service.calculate_growth_snapshot(
            user_id=str(user.id)
        )
        return {
            "success": True,
