...
def _calculate_relevance_score(text: str, query: str, field_weight: float = 1.0) -> float:
...
async def search_content(
    ...
    type: Optional[str] = Query(None, description="Filter by type: content, project, asset"),
    ...
    try:
        ...
        if not type or type == "content":
            # Build content query
            content_query = supabase.table("content").select(
                "*, projects(name)"
            ...
            for item in content_result.data or []:
                ...
                title_score = _calculate_relevance_score(item.get("title", ""), q, field_weight=2.0)
                
                # Check for matches in original_text
                text_score = _calculate_relevance_score(item.get("original_text", ""), q, field_weight=1.0)
                
                # Check for matches in source_url
                url_score = _calculate_relevance_score(item.get("source_url", ""), q, field_weight=0.5)
                ...
                if best_score > 0:
                    ...
                    if title_score >= text_score and title_score >= url_score:
                        ...
                        matched_text = _highlight_match(item.get("original_text", ""), q)
                    else:
                    ...
                    results.append(SearchResult(
                        ...
                        project_name=item.get("projects", {}).get("name") if item.get("projects") else None
                    ))
        
        # Search projects
        if not type or type == "project":
            project_query = supabase.table("projects").select("*").eq(
                "user_id", user_id
            ).is_("deleted_at", None)
            ...
            for item in project_result.data or []:
                # Check name match
                name_score = _calculate_relevance_score(item.get("name", ""), q, field_weight=2.0)
                
                # Check description match
                desc_score = _calculate_relevance_score(item.get("description", ""), q, field_weight=1.0)
...
async def get_search_suggestions(
    ...
    try:
        ...
        content_result = supabase.table("content").select("title").eq(
            "user_id", user_id
        ).is_("deleted_at", None).order("created_at", desc=True).limit(50).execute()
        
        # Get project names
        project_result = supabase.table("projects").select("name").eq(
            "user_id", user_id
