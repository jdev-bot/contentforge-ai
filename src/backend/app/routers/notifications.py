...
class EmailPreferencesUpdate(BaseModel):
    ...
    marketing_emails: bool = Field(
        default=True, description="Receive marketing and promotional emails"
    )
    usage_alerts: bool = Field(default=True, description="Receive usage limit alerts")
    weekly_digest: bool = Field(
        default=True, description="Receive weekly usage summaries"
    )
    feature_announcements: bool = Field(
        default=True, description="Receive new feature announcements"
    )
...
async def get_email_preferences(user=Depends(get_auth_user)):
    ...
    try:
        result = (
            ...
            .select("email_preferences, updated_at")
            .eq("id", str(user.id))
            .single()
            .execute()
        )
...
async def update_email_preferences(
    ...
    try:
        ...
        result = (
            supabase.table("profiles")
            .update({"email_preferences": prefs_dict, "updated_at": "now()"})
            .eq("id", str(user.id))
            .execute()
        )
