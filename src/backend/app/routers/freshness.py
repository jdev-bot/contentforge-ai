...
class FreshnessAnalyzeRequest(BaseModel):
    ...
    force_reanalyze: bool = Field(default=False, description="Force reanalysis even if recently analyzed")
...
async def analyze_content_freshness(
    ...
    try:
        ...
        if not request.force_reanalyze:
            existing = supabase.table("content_freshness_scores").select("*").eq("content_id", str(content_id)).eq("user_id", str(user.id)).execute()
            if existing.data:
                last_analyzed = existing.data[0].get("last_analyzed_at")
                if last_analyzed:
                    last_analyzed_dt = datetime.fromisoformat(last_analyzed.replace("Z", "+00:00"))
                    if datetime.now(last_analyzed_dt.tzinfo) - last_analyzed_dt < timedelta(hours=24):
                        ...
                        return FreshnessAnalyzeResponse(
                            ...
                            status=freshness_service.get_freshness_status(score_data["freshness_score"]),
        ...
        status_label = freshness_service.get_freshness_status(analysis["freshness_score"])
...
async def list_stale_content(
    ...
    try:
        ...
        result = supabase.table("content_freshness_scores").select("*, content!inner(title, created_at)").eq("user_id", str(user.id)).lt("freshness_score", threshold).order("freshness_score").range(offset, offset + page_size - 1).execute()
        ...
        # Get total count
        count_result = supabase.table("content_freshness_scores").select("count", count="exact").eq("user_id", str(user.id)).lt("freshness_score", threshold).execute()
...
async def bulk_analyze_content(
    ...
    try:
        ...
        if content_ids:
            ...
            content_query = supabase.table("content").select("*").eq("user_id", str(user.id)).in_("id", [str(cid) for cid in content_ids])
        else:
            # Analyze all user content
            content_query = supabase.table("content").select("*").eq("user_id", str(user.id))
...
async def get_freshness_dashboard(
    ...
    try:
        ...
        content_result = supabase.table("content").select("count", count="exact").eq("user_id", str(user.id)).execute()
        total_content = content_result.count or 0
        
        # Get all freshness scores
        scores_result = supabase.table("content_freshness_scores").select("*").eq("user_id", str(user.id)).execute()
        ...
        recent_result = supabase.table("content_freshness_scores").select("*").eq("user_id", str(user.id)).order("last_analyzed_at", desc=True).limit(5).execute()
        
        recent_analyses = []
        for score_data in recent_result.data:
            status_label = freshness_service.get_freshness_status(score_data["freshness_score"])
            recent_analyses.append(FreshnessScoreResponse(
                id=score_data["id"],
                content_id=score_data["content_id"],
        ...
        oldest_result = supabase.table("content_freshness_scores").select("*, content!inner(title, created_at)").eq("user_id", str(user.id)).order("age_days", desc=True).limit(5).execute()
        
        oldest_content = []
        for score_data in oldest_result.data:
            content_info = score_data.get("content", {})
            status_label = freshness_service.get_freshness_status(score_data["freshness_score"])
            oldest_content.append(StaleContentItem(
                content_id=score_data["content_id"],
                title=content_info.get("title", "Untitled"),
                freshness_score=score_data["freshness_score"],
                age_days=score_data["age_days"],
