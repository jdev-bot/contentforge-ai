...
def check_idempotency(
    idempotency_key: str, webhook_type: str, timeout_minutes: int = 60
) -> bool:
    """
    ...
    **Headers:**
    ...
    try:
        ...
        if payload.get("extracted_text"):
            ...
            word_count = payload.get("word_count") or len(
                payload["extracted_text"].split()
            )
...
async def get_webhook_logs(
    ...
    return WebhookLogResponse(
        logs=result.data or [], total=result.count or 0, page=page, limit=limit
    )
...
async def get_webhook_config(user=Depends(get_auth_user)):
    """
...
async def test_content_webhook(content_id: UUID, user=Depends(get_auth_user)):
    """
...
async def test_distribution_webhook(distribution_id: UUID, user=Depends(get_auth_user)):
    """
