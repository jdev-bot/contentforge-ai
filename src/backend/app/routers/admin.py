...
async def get_admin_errors(limit: int = 100, user=Depends(get_auth_user)):
    """
    ...
    try:
        ...
        return ErrorListResponse(total=len(error_entries), errors=error_entries)
...
async def get_admin_errors_summary(hours: int = 24, user=Depends(get_auth_user)):
    """
    ...
    try:
        ...
        return ErrorSummaryResponse(
            ...
            period_hours=hours,
            by_status_code={
                str(k): v for k, v in summary.get("by_status_code", {}).items()
            },
...
async def get_user_stats(user=Depends(get_auth_user)):
    """
    ...
    try:
        ...
        # Get total profiles
        profiles_result = (
            supabase.table("profiles").select("*", count="exact").execute()
        )
