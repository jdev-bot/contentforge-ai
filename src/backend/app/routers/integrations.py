...
class IntegrationCreate(BaseModel):
    ...
    is_active: bool = Field(
        default=True, description="Whether the integration is active"
    )
...
async def list_integrations(
    ...
    # Build query
    query = (
        supabase.table("integrations").select("*", count="exact").eq("user_id", user_id)
    )
    ...
    for item in result.data or []:
        ...
        integrations.append({**item, "config": masked_config})

    return IntegrationListResponse(
        integrations=integrations, total=result.count or len(integrations)
    )
...
async def delete_integration(integration_id: UUID, user=Depends(get_auth_user)):
    ...
    supabase.table("integrations").delete().eq("id", str(integration_id)).eq(
        "user_id", user_id
    ).execute()
...
async def get_integration_types(user=Depends(get_auth_user)):
    ...
    return IntegrationTypesResponse(types=types, event_types=AVAILABLE_EVENT_TYPES)
...
async def test_integration(integration_id: UUID, user=Depends(get_auth_user)):
    """
...
async def incoming_webhook(
    ...
    result = (
        supabase.table("integrations")
        .select("*")
        .eq("config->>incoming_token", token)
        .eq("is_active", True)
        .execute()
    )
...
async def list_webhook_deliveries(
    ...
    query = (
        ...
        .select("*", count="exact")
        .eq("webhook_id", str(integration_id))
    )
    ...
    return WebhookDeliveryListResponse(
        deliveries=result.data or [], total=result.count or 0, page=page, limit=limit
    )


@router.post("/integrations/{integration_id}/deliveries/{delivery_id}/retry")
async def retry_webhook_delivery(
    integration_id: UUID, delivery_id: UUID, user=Depends(get_auth_user)
):
    """
...
async def trigger_integration_event(
    ...
    return {
        "success": success,
        "message": message
        or ("Event sent successfully" if success else "Failed to send event"),
