...
async def export_user_data(user=Depends(get_auth_user)):
    ...
    try:
        ...
        # Get content items
        content_result = (
            supabase.table("content").select("*").eq("user_id", user_id).execute()
        )
        ...
        distributions_result = (
            supabase.table("distributions").select("*").eq("user_id", user_id).execute()
        )
        ...
        usage_result = (
            supabase.table("usage_logs").select("*").eq("user_id", user_id).execute()
        )
            ...
            "days_remaining": max(
                ...
                - (
                    ...
                    - datetime.fromisoformat(
                        deletion.get("requested_at").replace("Z", "+00:00")
                    )
                ).days,
            ),
        }
