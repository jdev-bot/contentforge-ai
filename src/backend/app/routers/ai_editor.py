...
class RewriteRequest(BaseModel):
    ...
    style: str = Field(default="neutral", description="neutral, persuasive, informative, storytelling, concise, descriptive")
    
    @validator('tone')
    def validate_tone(cls, v):
        valid_tones = ["casual", "professional", "witty", "formal", "friendly", "authoritative", "enthusiastic", "empathetic"]
    ...
    def validate_style(cls, v):
        valid_styles = ["neutral", "persuasive", "informative", "storytelling", "concise", "descriptive"]
...
class ExpandRequest(BaseModel):
    ...
    target_length: int = Field(default=2, ge=2, le=5, description="Length multiplier (2x, 3x, 4x, 5x)")
    focus_areas: List[str] = Field(default=[], description="Specific areas to focus expansion on")


class CondenseRequest(BaseModel):
    content: str = Field(..., min_length=20, description="Content to condense")
    target_percentage: int = Field(default=50, ge=20, le=80, description="Target percentage of original length")
    preserve_key_points: bool = Field(default=True, description="Whether to preserve key points")


class OptimizeRequest(BaseModel):
    content: str = Field(..., min_length=10, description="Content to optimize")
    platform: str = Field(..., description="twitter, linkedin, blog, newsletter, instagram, tiktok")
    include_hashtags: bool = Field(default=True, description="Include hashtags in optimized content")
    include_cta: bool = Field(default=True, description="Include call-to-action")
    
    @validator('platform')
    def validate_platform(cls, v):
        valid_platforms = ["twitter", "linkedin", "blog", "newsletter", "instagram", "tiktok"]
...
async def get_editor_history(
    ...
    try:
        query = supabase.table("ai_editor_history") \
            .select("*") \
