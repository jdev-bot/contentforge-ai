...
class GenerateContentRequest(BaseModel):
    ...
    platform: str = Field(
        default="twitter",
        description="Platform: twitter, linkedin, blog, newsletter, instagram, tiktok",
    )
    tone: str = Field(
        default="professional",
        description="Content tone: professional, casual, witty, formal, friendly",
    )
...
async def list_trending_topics(
    category: Optional[str] = Query(
        None, description="Filter by category (tech, business, entertainment, etc.)"
    ),
        ...
        for category, topics in topics_by_category.items():
            result.append(
                TrendCategoryResponse(
                    ...
                    topics=[
                        TrendingTopicResponse(**t) for t in topics[:10]
                    ],  # Top 10 per category
...
async def get_tracked_topics(user=Depends(get_auth_user)):
    """
...
async def untrack_topic(topic_id: UUID, user=Depends(get_auth_user)):
    """
        ...
        if topic_id:
            try:
                suggestion_data = {
                    ...
                    "tokens_used": len(content_data.get("content", "").split())
                    * 2,  # Rough estimate
                }
                result = (
                    supabase.table("trend_content_suggestions")
...
async def refresh_trending_topics(user=Depends(get_auth_user)):
    """
...
async def get_trend_details(topic_id: UUID, user=Depends(get_auth_user)):
    """
