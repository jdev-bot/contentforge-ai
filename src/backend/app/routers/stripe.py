...
async def get_subscription_status(user=Depends(get_auth_user)):
    ...
    try:
        ...
        local_plan = result.data.get("subscription_tier", "free") if result.data else "free"
        local_status = result.data.get("subscription_status", "inactive") if result.data else "inactive"
        ...
        if not customer_id:
            return {
                ...
                "monthly_usage_count": result.data.get("monthly_usage_count", 0) if result.data else 0,
                "monthly_usage_limit": result.data.get("monthly_usage_limit", 10) if result.data else 10,
            }
        ...
        if subscriptions.data:
            ...
            return {
                ...
                "monthly_usage_count": result.data.get("monthly_usage_count", 0) if result.data else 0,
                "monthly_usage_limit": 100 if plan == "starter" else (1000 if plan == "pro" else 10),
            }
        ...
        sub_status = result.data.get("subscription_status", "inactive") if result.data else "inactive"
        return {
            "status": sub_status,
            "plan": plan,
            "stripe_connected": False,
...
async def stripe_webhook(request: Request):
    ...
    try:
        ...
        # ============================================================
            ...
            if user_id:
                logger.info(f"Subscription updated for user {user_id}, status: {sub_status}")
                
                # Get plan from metadata or items
                plan = subscription.get("metadata", {}).get("plan")
                if not plan and subscription.get("items", {}).get("data"):
                    plan = get_plan_from_price_id(subscription["items"]["data"][0]["price"]["id"])
                plan = plan or "pro"
                ...
                supabase.table("profiles").update({
                    ...
                    "monthly_usage_limit": monthly_limit if our_status == "active" else 10,
                    "subscription_period_end": subscription.get("current_period_end"),
        ...
        # ============================================================
            ...
            result = supabase.table("profiles").select("id").eq("stripe_customer_id", customer.get("id")).execute()
            if result.data:
    ...
    try:
        ...
        if subscriptions.data:
            ...
            supabase.table("profiles").update({
                ...
                "subscription_status": "active" if sub.status == "active" else sub.status,
            ...
            return {"synced": True, "message": "No active subscription found, reset to free"}
