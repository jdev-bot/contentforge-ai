...
class ScheduleConfig(BaseModel):
    frequency: str = Field(
        ..., description="once, hourly, daily, weekly, monthly, custom"
    )
...
async def create_automation_rule(
    ...
    try:
        ...
        next_run_at = None
        if (
            rule_data.trigger_type == TriggerType.SCHEDULED_TIME
            and rule_data.schedule_config
        ):
            if (
                rule_data.schedule_config.frequency == "once"
                and rule_data.schedule_config.run_at
            ):
        ...
        rule_record = {
            ...
            "schedule_config": (
                rule_data.schedule_config.model_dump()
                if rule_data.schedule_config
                else None
            ),
            "webhook_config": (
                rule_data.webhook_config.model_dump()
                if rule_data.webhook_config
                else None
            ),
            "status": (
                AutomationRuleStatus.ACTIVE.value
                if rule_data.enabled
                else AutomationRuleStatus.PAUSED.value
            ),
...
async def list_automation_rules(
    ...
    try:
        query = (
            supabase.table("automation_rules").select("*").eq("user_id", str(user.id))
        )
...
async def toggle_automation_rule(rule_id: UUID, user=Depends(get_auth_user)):
    ...
    try:
        ...
        new_status = (
            AutomationRuleStatus.PAUSED.value
            if current_status == AutomationRuleStatus.ACTIVE.value
            else AutomationRuleStatus.ACTIVE.value
        )
...
async def run_automation_rule(
    ...
    try:
        ...
        background_tasks.add_task(
            execute_automation_action, rule, log_id, str(user.id), supabase
        )
...
async def list_automation_logs(
    ...
    try:
        query = (
            supabase.table("automation_logs").select("*").eq("user_id", str(user.id))
        )
...
async def create_webhook_endpoint(
    ...
    try:
        ...
        webhook_record = {
            ...
            "project_id": (
                str(webhook_data.project_id) if webhook_data.project_id else None
            ),
            "user_id": str(user.id),
...
async def get_publishing_queue(
    ...
    try:
        query = (
            supabase.table("publishing_queue").select("*").eq("user_id", str(user.id))
        )
...
async def bulk_schedule_content(
    ...
    try:
        ...
        return {
            ...
            "end_time": (
                start_time
                + timedelta(minutes=interval_minutes * (len(content_ids) - 1))
            ).isoformat(),
...
async def execute_automation_action(
    rule: Dict[str, Any], log_id: str, user_id: str, supabase
):
