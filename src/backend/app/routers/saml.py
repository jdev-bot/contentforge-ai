...
class SAMLProviderCreate(BaseModel):
    ...
    name: str = Field(
        ...
        min_length=1,
        max_length=100,
        description="Provider short name (okta, onelogin, azure-ad, custom)",
    )
    ...
    slo_url: Optional[str] = Field(
        None, description="IdP SLO URL (SingleLogoutService)"
    )
    metadata_url: Optional[str] = Field(
        None, description="IdP Metadata URL (auto-fetch configuration)"
    )
    metadata_xml: Optional[str] = Field(
        None, description="Raw IdP metadata XML (overrides metadata_url)"
    )
    certificate: Optional[str] = Field(
        None, description="IdP X.509 signing certificate (PEM or base64)"
    )
    attribute_mapping: Optional[Dict[str, str]] = Field(
        None, description="SAML attribute to local field mapping"
    )
...
class SAMLLoginRequest(BaseModel):
    ...
    relay_state: Optional[str] = Field(
        None, description="RelayState to pass through the flow"
    )
...
class SAMLLogoutRequest(BaseModel):
    ...
    session_index: Optional[str] = Field(
        None, description="Session index from assertion"
    )
...
async def list_saml_providers(
    user=Depends(get_auth_user), org_id: Optional[str] = None
):
...
async def update_saml_provider(
    provider_id: str, body: SAMLProviderUpdate, user=Depends(get_auth_user)
):
...
class AttributeMappingUpdate(BaseModel):
    attribute_mapping: Dict[str, str] = Field(
        ..., description="SAML attribute name to local field mapping"
    )


@router.patch(
    "/saml/providers/{provider_id}/attribute-mapping",
    response_model=SAMLProviderResponse,
)
async def update_attribute_mapping(
    provider_id: str, body: AttributeMappingUpdate, user=Depends(get_auth_user)
):
    """Update attribute mapping for a SAML provider."""
    updated = saml_service.update_provider(
        provider_id, attribute_mapping=body.attribute_mapping
    )
    if not updated:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="SAML provider not found"
...
async def initiate_saml_login(
    body: SAMLLoginRequest, user=Depends(get_auth_user) if False else None
):
