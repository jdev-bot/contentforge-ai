    ...
    **Response codes:**
    ...
    try:
        ...
        components["redis"] = ComponentStatus(
            ...
            details={
                "url": (
                    redis_url.replace("//", "//***@") if "@" in redis_url else redis_url
                )
            },
            last_checked=datetime.now(timezone.utc).isoformat(),
        )
    ...
    if n8n_url:
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0, follow_redirects=True) as client:
                ...
                response = await client.get(
                    n8n_url.replace("/webhook/", "/healthz").rsplit("/", 1)[0]
                    + "/healthz"
                )
    ...
    if disk_info.get("status") == "unhealthy":
        overall_status = "degraded"
        alerts.append(
            f"Disk space critically low: {disk_info.get('free_gb', 0):.1f}GB free"
        )
    ...
    for component_name in critical_components:
        if (
            components.get(component_name, ComponentStatus(status="unknown")).status
            == "unhealthy"
        ):
