...
class CreateScheduleRequest(BaseModel):
    ...
    asset_id: Optional[str] = None
    platform: str = Field(
        ..., description="Platform to publish to (twitter, linkedin, etc.)"
    )
    scheduled_at: datetime = Field(
        ..., description="When to publish (will be converted to UTC)"
    )
    asset_type: str = Field(
        default="post", description="Type of asset: post, thread, article"
    )
    settings: dict = Field(
        default_factory=dict, description="Platform-specific settings"
    )
    timezone: str = Field(
        default="UTC", description="User's timezone for the scheduled_at time"
    )
    content: Optional[str] = Field(
        default=None, description="Content to publish (caches from asset)"
    )
...
@router.post(
    "/schedule",
    response_model=ScheduledPostItem,
...
async def create_scheduled_post(
    ...
    try:
        ...
        result = scheduler_service.schedule_post(
            user_id=str(user.id), request=schedule_request, content=request.content
        )
...
async def update_scheduled_post(
    post_id: str, request: UpdateScheduleRequest, user=Depends(get_auth_user)
):
    """
    ...
    try:
        ...
        result = scheduler_service.update_scheduled_post(
            user_id=str(user.id), post_id=post_id, request=update_request
        )
...
async def cancel_scheduled_post(post_id: str, user=Depends(get_auth_user)):
    """
...
async def publish_scheduled_post_now(post_id: str, user=Depends(get_auth_user)):
    """
    ...
    """
    try:
        result = scheduler_service.publish_now(user_id=str(user.id), post_id=post_id)
...
async def bulk_create_scheduled_posts(
    requests: List[CreateScheduleRequest], user=Depends(get_auth_user)
):
    """
