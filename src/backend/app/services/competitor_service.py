...
class CompetitorService:
    ...
    MOCK_TOPICS = [
        "content marketing", "SEO", "social media", "branding", "AI tools",
        "productivity", "leadership", "startup growth", "personal branding",
        "digital marketing", "email marketing", "video content", "podcasting",
        "webinar marketing", "community building", "influencer marketing",
        "data analytics", "customer experience", "automation", "storytelling"
    ]
    ...
    def _generate_mock_content(self, platform: str, handle: str, count: int = 10) -> List[Dict[str, Any]]:
        ...
        for i in range(count):
            ...
            shares = int(base_engagement * random.uniform(0.1, 0.4)) if platform in ["twitter", "linkedin", "facebook"] else 0
            views = int(likes * random.uniform(5, 20)) if platform in ["instagram", "tiktok", "youtube"] else likes * 3
            ...
            content_item = {
                ...
                "sentiment": random.choice(["positive", "neutral", "positive", "positive"]),  # Skew positive
                "topics": [topic],
                "keywords": keywords,
    ...
    async def get_competitors(self, user_id: str, platform: Optional[str] = None) -> List[Dict[str, Any]]:
        """Get all competitors for a user."""
        query = self.supabase.table("competitors").select("*").eq("user_id", user_id).eq("is_active", True)
    ...
    async def get_competitor_by_id(self, competitor_id: str, user_id: str) -> Optional[Dict[str, Any]]:
        """Get a specific competitor by ID."""
        result = self.supabase.table("competitors").select("*").eq("id", competitor_id).eq("user_id", user_id).execute()
    ...
    async def remove_competitor(self, competitor_id: str, user_id: str) -> bool:
        ...
        result = self.supabase.table("competitors").update({"is_active": False}).eq("id", competitor_id).eq("user_id", user_id).execute()
    ...
    async def get_competitor_content(
        ...
        result = self.supabase.table("competitor_content").select("*").eq("competitor_id", competitor_id).order("published_at", desc=True).range(offset, offset + limit - 1).execute()
        return result.data or []
    
    async def refresh_competitor_data(self, competitor_id: str, user_id: str) -> Dict[str, Any]:
    ...
    async def _get_content_count(self, competitor_id: str) -> int:
        ...
        result = self.supabase.table("competitor_content").select("id", count="exact").eq("competitor_id", competitor_id).execute()
    ...
    async def get_performance_analysis(self, user_id: str) -> Dict[str, Any]:
        ...
        for comp in competitors:
            ...
            if recent_content.data:
                avg_likes = sum(c.get("likes", 0) for c in recent_content.data) / max(content_count, 1)
                avg_comments = sum(c.get("comments", 0) for c in recent_content.data) / max(content_count, 1)
                avg_shares = sum(c.get("shares", 0) for c in recent_content.data) / max(content_count, 1)
                avg_engagement = (avg_likes + avg_comments * 3 + avg_shares * 5) / max(comp["follower_count"], 1) * 100
            else:
                avg_engagement = 0
            ...
            analysis["platform_breakdown"][platform]["total_followers"] += comp["follower_count"]
        ...
        analysis["aggregated_metrics"] = {
            ...
            "avg_followers_per_competitor": round(total_followers / len(competitors), 0),
            "avg_engagement_rate": round(total_engagement / len(competitors), 2),
            "total_content_last_30_days": total_content,
        }
        
        # Generate insights
        analysis["insights"] = self._generate_insights(analysis["competitors"], analysis["aggregated_metrics"])
        
        return analysis
    
    def _generate_insights(self, competitors: List[Dict], metrics: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        by_engagement = sorted(competitors, key=lambda x: x["avg_engagement_rate"], reverse=True)
        ...
        by_activity = sorted(competitors, key=lambda x: x["content_last_30_days"], reverse=True)
    ...
    async def identify_content_gaps(self, user_id: str) -> Dict[str, Any]:
        ...
        user_topics = set()
        try:
            user_content = self.supabase.table("content").select("keywords").eq("user_id", user_id).execute()
        ...
        for topic, count in all_topics.most_common(20):
            ...
            if not has_content or count > 3:  # Gap if user has none OR competitors are very active
                gap = {
                    "user_id": user_id,
                    "topic": topic,
        ...
        for gap in gaps[:10]:  # Store top 10
            try:
                result = self.supabase.table("content_gaps").upsert(gap, on_conflict="user_id,topic").execute()
                if result.data:
    ...
    async def get_content_gaps(self, user_id: str, min_opportunity: int = 0) -> List[Dict[str, Any]]:
        """Get stored content gaps for a user."""
        query = self.supabase.table("content_gaps").select("*").eq("user_id", user_id).eq("is_addressed", False)
    ...
    async def analyze_topic_overlap(self, user_id: str) -> Dict[str, Any]:
        ...
        comp_topics = Counter()
        for comp in competitors:
            content = self.supabase.table("competitor_content").select("topics").eq("competitor_id", comp["id"]).execute()
        ...
        return {
            ...
            "overlap_percentage": round(len(overlap) / max(len(comp_topics), 1) * 100, 1),
            "shared_topics": list(overlap),
            "competitor_only_topics": list(unique_to_comp)[:10],
            "user_only_topics": list(unique_to_user)[:10],
            "recommendation": "Focus on competitor-only topics to close content gaps" if len(unique_to_comp) > 5 else "Good topic coverage",
        }
    ...
    async def get_benchmark_comparison(self, user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "competitor_avg": {
                ...
                "content_per_month": round(comp_metrics["total_content_last_30_days"] / max(analysis["competitor_count"], 1), 1),
                "engagement_rate": comp_metrics["avg_engagement_rate"],
            },
            "comparison": {
                "follower_gap": int(comp_metrics["avg_followers_per_competitor"] - user_metrics["follower_count"]),
                "content_gap": round(comp_metrics["total_content_last_30_days"] / max(analysis["competitor_count"], 1) - user_metrics["content_count_30d"], 1),
                "engagement_comparison": round(user_metrics["avg_engagement_rate"] - comp_metrics["avg_engagement_rate"], 2),
            },
            "percentile": {
                "followers": self._calculate_percentile(user_metrics["follower_count"], [c["follower_count"] for c in analysis["competitors"]]),
                "engagement": self._calculate_percentile(user_metrics["avg_engagement_rate"], [c["avg_engagement_rate"] for c in analysis["competitors"]]),
            },
        }
    ...
    async def fetch_all_competitor_data(self) -> Dict[str, Any]:
        ...
        result = self.supabase.table("competitors").select("*").eq("is_active", True).execute()
    ...
    async def generate_gap_analysis_for_all_users(self) -> Dict[str, Any]:
        ...
        result = self.supabase.table("competitors").select("user_id").eq("is_active", True).execute()
