...
class SchedulerService:
    ...
    def get_scheduled_post(self, user_id: str, post_id: str) -> Optional[ScheduledPostResponse]:
        """Get a specific scheduled post."""
        try:
            result = self.supabase.table("scheduled_posts").select("*").eq("id", post_id).eq("user_id", user_id).single().execute()
    ...
    def update_scheduled_post(
        ...
        try:
            ...
            result = self.supabase.table("scheduled_posts").update(update_data).eq("id", post_id).eq("user_id", user_id).execute()
    ...
    def cancel_scheduled_post(self, user_id: str, post_id: str) -> bool:
        ...
        try:
            ...
            result = self.supabase.table("scheduled_posts").update({"status": "cancelled"}).eq("id", post_id).eq("user_id", user_id).execute()
    ...
    def publish_now(self, user_id: str, post_id: str) -> Optional[ScheduledPostResponse]:
    ...
    def get_pending_posts_due(self, batch_size: int = 100) -> List[ScheduledPostResponse]:
        ...
        try:
            ...
            return [ScheduledPostResponse(**item) for item in result.data] if result.data else []
            
        except Exception as e:
            logger.error(f"Failed to get pending posts: {e}")
            return []
    
    def get_failed_posts_for_retry(self, batch_size: int = 50) -> List[ScheduledPostResponse]:
        """Get failed posts eligible for retry."""
        try:
            result = self.supabase.table("scheduled_posts").select("*").eq("status", "failed").lt("retry_count", 3).limit(batch_size).execute()
    ...
    def get_scheduler_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for status in statuses:
                result = self.supabase.table("scheduled_posts").select("id", count="exact").eq("user_id", user_id).eq("status", status).execute()
...
def publish_scheduled_post(self: Task, post_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Get post details
        result = supabase.table("scheduled_posts").select("*").eq("id", post_id).single().execute()
        ...
        # Mark as processing
        supabase.table("scheduled_posts").update({"status": "processing"}).eq("id", post_id).execute()
        ...
        try:
            supabase = get_supabase_client()
            current = supabase.table("scheduled_posts").select("retry_count").eq("id", post_id).single().execute()
            ...
            if current.data:
                ...
                supabase.table("scheduled_posts").update(update_data).eq("id", post_id).execute()
                
                # Retry if under max retries
                if new_retry_count < 3:
                    logger.info(f"Retrying post {post_id} (attempt {new_retry_count}/3)")
                    raise self.retry(exc=e, countdown=60 * new_retry_count)  # Exponential backoff
...
def retry_failed_posts() -> Dict[str, Any]:
    ...
    try:
        ...
        if not failed_posts:
            return {"status": "success", "retried": 0, "message": "No failed posts to retry"}
