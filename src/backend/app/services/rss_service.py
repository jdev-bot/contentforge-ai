...
class RSSService:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout, follow_redirects=True) as client:
                response = await client.get(url, headers={
                    "User-Agent": "ContentForge AI RSS Reader/1.0"
                })
    ...
    async def fetch_feed(self, feed_id: str, user_id: str) -> Dict[str, Any]:
        ...
        try:
            # Get feed details
            feed_result = supabase.table("rss_feeds").select("*").eq("id", feed_id).single().execute()
            ...
            async with httpx.AsyncClient(timeout=self.timeout, follow_redirects=True) as client:
                ...
                for entry in parsed.entries:
                    try:
                        ...
                        existing = supabase.table("rss_entries").select("id").eq("feed_id", feed_id).eq("external_id", external_id).execute()
                        ...
                        entry_data = {
                            ...
                            "title": title[:500] if title else "Untitled",  # Limit title length
    ...
    async def _auto_create_content_for_feed(self, feed_id: str, user_id: str, supabase):
        ...
        try:
            ...
            entries_result = supabase.table("rss_entries").select("*").eq("feed_id", feed_id).eq("processed", False).execute()
            ...
            for entry in entries_result.data:
                try:
                    ...
                    logger.error(f"Error auto-creating content for entry {entry['id']}: {e}")
                    continue
    ...
    async def import_entry(
        ...
        try:
            ...
            if not title or not content:
                entry_result = supabase.table("rss_entries").select("*").eq("id", entry_id).single().execute()
    ...
    async def fetch_all_active_feeds(self) -> Dict[str, Any]:
        ...
        try:
            ...
            feeds_result = supabase.table("rss_feeds").select("*").eq("status", "active").execute()
            ...
            for feed in feeds:
                ...
                if last_fetched:
                    ...
                    if frequency == "hourly" and minutes_since_fetch < 60:
                        continue
                    elif frequency == "daily" and minutes_since_fetch < 1440:  # 24 hours
                        continue
