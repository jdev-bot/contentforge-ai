...
class SSOService:
    ...
    def list_providers(self) -> List[Dict[str, Any]]:
        ...
        result = self.supabase.table("sso_providers").select("*").order("created_at", desc=False).execute()
        return result.data or []

    def get_provider(self, provider_id: str) -> Optional[Dict[str, Any]]:
        """Get a single SSO provider by ID."""
        result = self.supabase.table("sso_providers").select("*").eq("id", provider_id).single().execute()
        return result.data
    ...
    def get_provider_by_name(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        result = self.supabase.table("sso_providers").select("*").eq("name", name).single().execute()
        return result.data

    def create_provider(self, *, name: str, display_name: str, client_id: str,
                        client_secret: str, discovery_url: Optional[str] = None,
    ...
    def update_provider(self, provider_id: str, **fields) -> Optional[Dict[str, Any]]:
        ...
        result = self.supabase.table("sso_providers").update(fields).eq("id", provider_id).execute()
        if not result.data:
            return None
        return result.data[0]
    ...
    def delete_provider(self, provider_id: str) -> bool:
        ...
        result = self.supabase.table("sso_providers").delete().eq("id", provider_id).execute()
    ...
    def _discover_oidc_endpoints(self, provider: Dict[str, Any]) -> Dict[str, str]:
        ...
        if not discovery_url:
            raise ValueError(f"Provider '{provider['name']}' has no discovery URL and no explicit endpoints configured")
    ...
    def initiate_login(self, *, provider_name: str, redirect_uri: str,
    ...
    def handle_callback(self, *, state: str, code: str) -> Dict[str, Any]:
        ...
        """
        # Look up state
        state_result = self.supabase.table("sso_states").select("*").eq("state", state).single().execute()
    ...
    def _exchange_code(self, *, token_endpoint: str, code: str,
    ...
    def _extract_claims(self, *, token_response: Dict[str, Any],
    ...
    def _find_sso_identity(self, provider_name: str, subject: str) -> Optional[Dict[str, Any]]:
        """Find an existing SSO identity."""
        result = self.supabase.table("sso_identities").select("*").eq("provider", provider_name).eq("subject", subject).single().execute()
        return result.data

    def _create_sso_identity(self, *, user_id: str, provider_name: str,
                              subject: str, email: Optional[str],
    ...
    def list_user_identities(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        result = self.supabase.table("sso_identities").select("id, provider, email, full_name, created_at").eq("user_id", user_id).execute()
        return result.data or []

    def unlink_identity(self, identity_id: str, user_id: str) -> bool:
        """Unlink an SSO identity from a user account."""
        result = self.supabase.table("sso_identities").delete().eq("id", identity_id).eq("user_id", user_id).execute()
        return len(result.data) > 0
    ...
    def _provision_user(self, *, email: str, full_name: str,
                         provider_name: str, subject: str) -> Dict[str, Any]:
        """
