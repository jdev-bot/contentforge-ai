...
class BaseIntegrationService(ABC):
    ...
    def __init__(
        self, integration_id: Optional[str] = None, config: Optional[Dict] = None
    ):
...
class ZapierService(BaseIntegrationService):
    ...
    async def test_connection(self) -> tuple[bool, Optional[str]]:
        ...
        return await self._send_payload(
            test_payload, event_type="test.ping", is_test=True
        )
        ...
        while attempts < max_retries:
            try:
                ...
                if response.status_code < 400:
                    ...
                    return (
                        False,
                        f"Webhook returned HTTP {response.status_code}: {response.text[:500]}",
                    )
...
class WebhookService(BaseIntegrationService):
    ...
    def __init__(
        ...
        self.signature_header = self.config.get(
            "signature_header", "X-Webhook-Signature"
        )
        self.signature_format = self.config.get(
            "signature_format", "hex"
        )  # hex or base64
        self.hash_algorithm = self.config.get(
            "hash_algorithm", "sha256"
        )  # sha256, sha512
        self.event_filter = self.config.get(
            "event_filter", []
        )  # Empty list = all events
        self.payload_transform = self.config.get(
            "payload_transform", {}
    ...
    def _generate_signature(self, payload: Dict[str, Any]) -> str:
        ...
        if self.hash_algorithm == "sha512":
            sig = hmac.new(
                self.secret.encode("utf-8"), payload_str.encode("utf-8"), hashlib.sha512
            ).digest()
        else:
            sig = hmac.new(
        ...
        while attempts < max_retries:
            try:
                headers = {
                    ...
                    "X-Webhook-ID": (
                        str(self.integration_id) if self.integration_id else ""
                    ),
                }
...
class WordPressService(BaseIntegrationService):
    ...
    def __init__(
        ...
        self.default_status = self.config.get(
    ...
    async def test_connection(self) -> tuple[bool, Optional[str]]:
        ...
        try:
            ...
            response = requests.get(
                f"{self.site_url}/wp-json/wp/v2/users/me", headers=headers, timeout=30
            )
            ...
            if response.status_code == 200:
                ...
                return (
                    False,
                    f"WordPress returned HTTP {response.status_code}: {response.text[:500]}",
                )
            ...
            },
            {
                ...
                "config_schema": {
                    ...
                    "event_filter": {
                        "type": "array",
                        "required": False,
                        "label": "Event Filter (leave empty for all)",
                    },
                    "custom_headers": {
                        "type": "object",
                        "required": False,
            ...
            },
            {
                ...
                "config_schema": {
                    ...
                    "application_password": {
                        ...
                        "help": "Generate in WordPress Admin > Users > Profile > Application Passwords",
                    },
                    "default_status": {
                        "type": "string",
                        "required": False,
...
AVAILABLE_EVENT_TYPES = [
    ...
    {"type": "content.published", "description": "When content is published"},
    {
        ...
        "description": "When content distribution completes",
    },
