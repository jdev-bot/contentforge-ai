...
class AudienceService:
    ...
    def get_growth_metrics(
        self, user_id: str, platform: Optional[str] = None, days: int = 30
    ) -> Dict[str, Any]:
        """Get growth metrics for a user."""
        since = (datetime.now(timezone.utc) - timedelta(days=days)).isoformat()

        query = (
            self.supabase.table("audience_metrics")
            .select("*")
            .eq("user_id", user_id)
        ...
        return {
            "metrics": metrics,
            "growth_rates": {"7d": growth_7d, "30d": growth_30d, "90d": growth_90d},
    ...
    def get_latest_snapshot(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        result = (
            self.supabase.table("growth_snapshots")
            .select("*")
            .eq("user_id", user_id)
            .order("recorded_at", desc=True)
            .limit(1)
    ...
    def calculate_growth_snapshot(self, user_id: str) -> Dict[str, Any]:
    ...
    def generate_insights(self, user_id: str, days: int = 30) -> Dict[str, Any]:
        ...
        return {
            "summary": {
                "total_followers": (
                    snapshot.get("total_followers", 0) if snapshot else 0
                ),
                "growth_rate_30d": metrics_data.get("growth_rates", {}).get("30d", 0),
                "engagement_rate": (
                    snapshot.get("engagement_rate", 0) if snapshot else 0
                ),
            },
            "trends": trends,
    ...
    def _get_followers_at_date(
        self, user_id: str, platform: Optional[str], days_ago: int
    ) -> int:
    ...
    def _get_current_followers(self, user_id: str, platform: str) -> int:
    ...
    def _get_total_followers(self, user_id: str) -> int:
        """Get total followers across all platforms."""
        result = self.supabase.rpc(
            "get_total_followers", {"p_user_id": user_id}
        ).execute()
    ...
    def _get_current_totals(self, user_id: str) -> Dict[str, int]:
        """Get current totals by platform."""
        result = (
            self.supabase.table("audience_metrics")
    ...
    def _analyze_trends(self, user_id: str, days: int) -> Dict[str, Any]:
        ...
        return {"trend": trend, "direction": direction, "weekly_data": weekly_totals}

    def _compare_periods(self, user_id: str, days: int) -> Dict[str, Any]:
        """Compare current period to previous period."""
        current_growth = self._calculate_follower_growth(user_id, None, days)
        previous_growth = (
            self._calculate_follower_growth(user_id, None, days * 2) - current_growth
        )
        ...
        if previous_growth == 0:
            ...
            change_percent = (
                (current_growth - previous_growth) / abs(previous_growth)
            ) * 100
    ...
    def _predict_growth(self, user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "projected_followers_30d": round(
                current_followers + (weighted_daily_rate * 30)
            ),
            "growth_rate": round(weighted_daily_rate, 2),
            "confidence": (
                "medium" if abs(daily_rate_7d - daily_rate_30d) < 10 else "low"
            ),
        }
...
Provide a brief summary (2-3 sentences) highlighting the key insight and one actionable recommendation."""
    ...
    def _generate_recommendations(
        self, trends: Dict[str, Any], platforms: List[Dict[str, Any]]
    ) -> List[str]:
        ...
        if trend == "declining":
            recommendations.append(
                "Your audience growth is declining. Consider increasing posting frequency or diversifying content types."
            )
        elif trend == "stable":
            recommendations.append(
                "Your growth is stable. Experiment with new content formats to accelerate growth."
            )
        elif trend == "accelerating":
            recommendations.append(
                "Great momentum! Double down on your current content strategy."
            )
        ...
        for platform in platforms:
            ...
            if growth_30d < 10:
                recommendations.append(
                    f"{platform.get('platform', 'Unknown')} growth is slow. Review your posting strategy for this platform."
                )
