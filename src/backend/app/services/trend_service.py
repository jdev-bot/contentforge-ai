...
class TrendService:
    ...
    async def fetch_mock_trending_data(
    ...
    def _generate_sample_content(
        ...
        return [
            ...
            for source, text in zip(
                random.sample(
                    self.SOURCES, min(len(content_templates), len(self.SOURCES))
                ),
...
{{
                ...
                trend.update(
                    {
                        ...
                        "trend_score": analysis.get(
                            "relevance_score", trend["trend_score"]
                        ),
                        "content_opportunities": analysis.get(
    ...
    async def save_trends_to_db(self, trends: List[Dict[str, Any]]) -> int:
        ...
        for trend in trends:
            try:
                ...
                existing = (
                    self.supabase.table("trending_topics")
                    .select("id")
                ...
                if existing.data:
                    ...
                    self.supabase.table("trending_topics").update(update_data).eq(
                        "id", trend_id
                    ).execute()
                else:
                    # Insert new trend
    ...
    async def get_trending_topics(
        self, category: Optional[str] = None, limit: int = 20, min_score: float = 0.0
    ) -> List[Dict[str, Any]]:
    ...
    async def track_topic_for_user(
        self, user_id: str, topic_id: str, relevance_score: float = 0.0
    ) -> bool:
    ...
    async def get_user_tracked_topics(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            result = (
                self.supabase.table("user_topic_interests")
    ...
    async def generate_content_from_trend(
        self, topic: str, category: str, platform: str, tone: str = "professional"
    ) -> Dict[str, Any]:
    ...
    def _parse_generated_content(self, result: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if "HEADLINE:" in result:
                headline_section = (
                    result.split("HEADLINE:")[1].split("ANGLES:")[0]
                    if "ANGLES:" in result
                    else result.split("HEADLINE:")[1]
                )
            ...
            if "CONTENT:" in result:
                content_section = (
                    result.split("CONTENT:")[1].split("HASHTAGS:")[0]
                    if "HASHTAGS:" in result
                    else result.split("CONTENT:")[1]
                )
            ...
            if "ANGLES:" in result:
                angles_section = (
                    result.split("ANGLES:")[1].split("CONTENT:")[0]
                    if "CONTENT:" in result
                    else result.split("ANGLES:")[1]
                )
            ...
            if "HASHTAGS:" in result:
                hashtags_section = (
                    result.split("HASHTAGS:")[1].split("CTA:")[0]
                    if "CTA:" in result
                    else result.split("HASHTAGS:")[1]
                )
                hashtags_text = hashtags_section.strip()
                data["hashtags"] = [
                    h.strip() for h in hashtags_text.split() if h.startswith("#")
                ]
