...
def update_email_status(email_id: str, status: str, error_message: Optional[str] = None):
    ...
    try:
        ...
        supabase.table("email_tracking").update(update_data).eq("id", email_id).execute()
    except Exception as e:
...
def send_email_task(
    ...
    try:
        ...
        try:
            ...
            raise EmailTaskError(f"Failed to send email after {self.max_retries} retries")
...
def send_usage_alert_task(user_id: str, email: str, user_name: str):
    ...
    try:
        ...
        alert_result = supabase.table("email_tracking").select("*").eq(
            "user_id", user_id
        ).eq("template_type", EmailTemplateType.USAGE_ALERT.value).order(
            "sent_at", desc=True
        ).limit(1).execute()
        ...
        if alert_result.data:
            ...
            if last_sent:
                ...
                if datetime.now(last_sent_date.tzinfo) - last_sent_date < timedelta(days=7):
...
def send_weekly_digests():
    ...
    try:
        ...
        ).eq("email_preferences->weekly_digest", True).execute()
        ...
        for user in result.data:
            try:
                ...
                logger.error(f"Failed to send weekly digest to {user.get('email')}: {e}")
                continue
...
def send_abandoned_cart_reminder(
    ...
    try:
        ...
        supabase = get_supabase_client()
        result = supabase.table("profiles").select("email_verified").eq(
            "id", user_id
        ).single().execute()
