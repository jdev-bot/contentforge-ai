...
def fetch_rss_feeds_task(self):
    ...
    try:
        ...
        logger.info(f"RSS fetch task completed: {processed}/{total_feeds} feeds processed")
        ...
        try:
            ...
            raise RSSTaskError(f"Failed to fetch RSS feeds after {self.max_retries} retries")
...
def process_rss_entry_task(self, entry_id: str, user_id: str, project_id: Optional[str] = None):
    """
    ...
    try:
        import asyncio
        result = asyncio.run(rss_service.import_entry(
            entry_id=entry_id,
            user_id=user_id,
...
def retry_failed_feeds_task():
    ...
    try:
        ...
        feeds_result = supabase.table("rss_feeds").select("id, user_id").eq("status", "error").execute()
