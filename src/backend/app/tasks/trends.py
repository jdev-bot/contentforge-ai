    ...
    try:
        ...
        try:
            deleted_count = loop.run_until_complete(
                trend_service._cleanup_expired_trends()
            )
        finally:
            loop.close()
            ...
            for trend in top_trends:
                for platform in platforms:
                    try:
                        ...
                        logger.warning(
                            f"Failed to generate suggestion for {trend['topic']} on {platform}: {e}"
                        )
                        continue
...
def notify_users_of_relevant_trends_task(self):
    ...
    try:
        ...
        try:
            ...
            users_result = (
                supabase.table("user_topic_interests").select("user_id").execute()
            )
            ...
            for user_id in user_ids:
                try:
                    relevant = loop.run_until_complete(
                        trend_service.get_relevant_topics_for_user(
                    ...
                    new_relevant = [
                        t for t in relevant if t.get("relevance_score", 0) > 0.7
                    ]
                    ...
                    if new_relevant:
                        ...
                        # For now, just log
                        logger.info(
                            f"Found {len(new_relevant)} relevant trends for user {user_id}"
                        )
                        notified_count += 1
