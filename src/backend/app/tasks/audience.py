...
def calculate_growth_metrics(self):
    ...
    try:
        ...
        logger.info(
            f"Growth metrics calculation complete. Processed: {processed}, Failed: {failed}"
        )
        return {
            "success": True,
            "processed": processed,
...
def calculate_user_growth_metrics(self, user_id: str):
    ...
    try:
        ...
        return {"success": True, "user_id": user_id, "snapshot": snapshot}
    except Exception as e:
        logger.error(f"Failed to calculate metrics for user {user_id}: {e}")
        self.retry(countdown=60 * (2**self.request.retries))
...
def process_platform_webhook(self, platform: str, payload: dict):
    ...
    try:
        ...
        return {"success": True, "platform": platform, "recorded": recorded}
...
def cleanup_old_audience_metrics(days: int = 365):
    ...
    try:
        ...
        supabase.table("growth_snapshots").delete().lt(
            "recorded_at", snapshot_cutoff
        ).execute()

        logger.info(f"Cleaned up {deleted_count} old metrics")
        return {"success": True, "deleted_metrics": deleted_count}
