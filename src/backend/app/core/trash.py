...
def soft_delete_content(content_id: str, user_id: str) -> bool:
    ...
    try:
        ...
        supabase.table("content").update(
            {"deleted_at": now.isoformat(), "updated_at": now.isoformat()}
...
def restore_from_trash(item_id: str, user_id: str) -> bool:
    ...
    try:
        ...
        supabase.table("trash").update({"restored_at": now.isoformat()}).eq(
            "id", item_id
...
def permanently_delete(item_id: str, user_id: str) -> bool:
    ...
    try:
        ...
        # Delete from trash
        supabase.table("trash").delete().eq("id", item_id).eq(
            "user_id", user_id
        ).execute()
