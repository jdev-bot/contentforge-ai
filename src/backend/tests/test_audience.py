...
class TestAudienceService:
    ...
    def test_get_growth_metrics(self, audience_service, mock_supabase, mock_user):
        ...
        mock_supabase.table.return_value.select.return_value.eq.return_value.gte.return_value.order.return_value.execute.return_value = (
            mock_response
        )
    ...
    def test_get_platforms_metrics(self, audience_service, mock_supabase, mock_user):
        ...
        with patch.object(
            audience_service, "_get_current_followers"
        ) as mock_followers, patch.object(
            audience_service, "_calculate_follower_growth"
        ) as mock_growth:
    ...
    def test_get_historical_data(self, audience_service, mock_supabase, mock_user):
        ...
        mock_supabase.table.return_value.select.return_value.eq.return_value.eq.return_value.gte.return_value.order.return_value.execute.return_value = (
            mock_response
        )

        # Act
        result = audience_service.get_historical_data(
            user_id=mock_user.id, metric_type="followers", days=30
        )
    ...
    def test_calculate_growth_snapshot(
        self, audience_service, mock_supabase, mock_user
    ):
        ...
        with patch.object(
            audience_service, "_get_total_followers"
        ) as mock_total, patch.object(
            audience_service, "_calculate_follower_growth"
        ) as mock_growth, patch.object(
            audience_service, "_calculate_engagement_rate"
        ) as mock_engagement, patch.object(
            audience_service, "_identify_top_performing_content"
        ) as mock_top:
    ...
    def test_calculate_growth_rate_zero_past(
        self, audience_service, mock_supabase, mock_user
    ):
    ...
    def test_get_current_followers(self, audience_service, mock_supabase, mock_user):
        ...
        mock_supabase.table.return_value.select.return_value.eq.return_value.eq.return_value.eq.return_value.order.return_value.limit.return_value.execute.return_value = (
            mock_response
        )
    ...
    def test_calculate_follower_growth(
        self, audience_service, mock_supabase, mock_user
    ):
        ...
        with patch.object(audience_service, "_get_followers_at_date") as mock_followers:
            ...
            # Act
            result = audience_service._calculate_follower_growth(
                mock_user.id, "twitter", 7
            )
    ...
    def test_generate_insights(self, audience_service, mock_supabase, mock_user):
        ...
        # Arrange
        with patch.object(
            audience_service, "get_growth_metrics"
        ) as mock_growth, patch.object(
            audience_service, "get_platforms_metrics"
        ) as mock_platforms, patch.object(
            audience_service, "get_latest_snapshot"
        ) as mock_snapshot, patch.object(
            audience_service, "_analyze_trends"
        ) as mock_trends, patch.object(
            audience_service, "_compare_periods"
        ) as mock_compare, patch.object(
            audience_service, "_predict_growth"
        ) as mock_predict:
            ...
            mock_compare.return_value = {
                "current_period_growth": 50,
                "previous_period_growth": 30,
...
class TestAudienceRouter:
    ...
    def test_get_historical_data_endpoint(self, client, mock_user):
        ...
        with patch("app.routers.audience.audience_service") as mock_service:
            ...
            response = client.get(
                "/api/v1/audience/history?days=90&metric_type=followers"
            )
...
class TestAudienceTasks:
    ...
    def test_calculate_growth_metrics_task(self):
        ...
        with patch(
            "app.tasks.audience.get_supabase_admin_client"
        ) as mock_get_client, patch(
            "app.tasks.audience.audience_service"
        ) as mock_service:
    ...
    def test_process_platform_webhook_task(self):
        ...
        with patch("app.tasks.audience.audience_service") as mock_service:
            ...
            payload = {"user_id": "user-1", "metric_type": "followers", "value": 1000}
...
class TestDatabaseIntegration:
    ...
    def test_migration_applies(self):
        ...
        migration_path = os.path.join(
            os.path.dirname(__file__), "..", "migrations", "015_audience_metrics.sql"
        )
