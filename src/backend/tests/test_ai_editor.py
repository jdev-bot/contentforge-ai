...
class TestGroqServiceEditorMethods:
    ...
    async def test_rewrite_content_different_tones(self, groq_service_instance):
        ...
        tones = ["casual", "witty", "formal", "friendly", "authoritative", "enthusiastic", "empathetic"]
        ...
        for tone in tones:
            ...
            mock_response.json.return_value = {
                "choices": [{
                    "message": {
                        "content": f"Rewritten in {tone} tone"
                    }
                }]
            }
    ...
    async def test_optimize_content_all_platforms(self, groq_service_instance):
        ...
        for platform in platforms:
            ...
            mock_response.json.return_value = {
                "choices": [{
                    "message": {
                        "content": f"Optimized for {platform}"
                    }
                }]
            }
...
class TestEditorEndpoints:
    ...
    async def test_rewrite_content_endpoint(self, mock_user, mock_usage_stats):
        ...
        with patch("app.routers.ai_editor.check_and_increment_usage") as mock_usage:
            ...
            with patch.object(groq_service, "rewrite_content", return_value=(mock_result, 150)):
                request = RewriteRequest(
                ...
                with patch("app.routers.ai_editor.get_auth_user", return_value=mock_user):
                    with patch("app.routers.ai_editor.enforce_subscription_limit", return_value=mock_usage_stats):
    ...
    async def test_expand_content_endpoint(self, mock_user, mock_usage_stats):
        ...
        with patch("app.routers.ai_editor.check_and_increment_usage") as mock_usage:
            ...
            with patch.object(groq_service, "expand_content", return_value=(expanded, 200)):
                request = ExpandRequest(
                    content=original,
                    target_length=3,
    ...
    async def test_condense_content_endpoint(self, mock_user, mock_usage_stats):
        ...
        with patch("app.routers.ai_editor.check_and_increment_usage") as mock_usage:
            ...
            with patch.object(groq_service, "condense_content", return_value=(condensed, 100)):
                request = CondenseRequest(
                    content=original,
                    target_percentage=50,
                    preserve_key_points=True,
    ...
    async def test_optimize_content_endpoint(self, mock_user, mock_usage_stats):
        ...
        with patch("app.routers.ai_editor.check_and_increment_usage") as mock_usage:
            ...
            with patch.object(groq_service, "optimize_content", return_value=result_dict):
                request = OptimizeRequest(
                    content="Original content",
                    platform="twitter",
                    include_hashtags=True,
