...
class TestDistributions:
    ...
    def mock_auth_dependencies(self):
        ...
        with patch("app.routers.distributions.get_auth_user") as mock_auth, \
             patch("app.routers.auth.get_supabase_client") as mock_supabase_auth, \
             patch("app.routers.distributions.get_supabase_client") as mock_supabase_dist:
    ...
    def test_create_distribution_success(self, mock_auth_dependencies):
        ...
        mock_supabase.table.return_value.select.return_value.eq.return_value.eq.return_value.single.return_value.execute.return_value = asset_result
        ...
        response = client.post("/api/v1/distributions", json={
            "asset_id": str(asset_id),
            "platform": "twitter",
            "scheduled_at": None
    ...
    def test_list_distributions(self, mock_auth_dependencies):
        ...
        response = client.get("/api/v1/distributions", headers={"Authorization": "Bearer test-token"})
    ...
    def test_get_distribution_success(self, mock_auth_dependencies):
        ...
        response = client.get(f"/api/v1/distributions/{distribution_id}", headers={"Authorization": "Bearer test-token"})
    ...
    def test_cancel_distribution_success(self, mock_auth_dependencies):
        ...
        mock_supabase.table.return_value.select.return_value.eq.return_value.eq.return_value.single.return_value.execute.return_value = existing_result
        
        # Mock delete
        mock_supabase.table.return_value.delete.return_value.eq.return_value.execute.return_value = MagicMock()
        
        response = client.delete(f"/api/v1/distributions/{distribution_id}", headers={"Authorization": "Bearer test-token"})
    ...
    def test_cancel_published_distribution_fails(self, mock_auth_dependencies):
        ...
        assert "Cannot delete already published distribution" in response.json()["detail"]
