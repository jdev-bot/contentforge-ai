...
class TestFreshnessServiceTrendScore:
    ...
    def test_some_trending_keywords(self):
        ...
        service = FreshnessService()
        content = {"original_text": "ai automation workflow", "title": "AI Automation"}
        score = service.calculate_trend_score(content)
    ...
    def test_few_trending_keywords(self):
        ...
        service = FreshnessService()
        content = {"original_text": "ai", "title": "AI"}
        score = service.calculate_trend_score(content)
    ...
    def test_trend_score_case_insensitive(self):
        ...
        service = FreshnessService()
        content = {"original_text": "AI MACHINE LEARNING", "title": "AI"}
        score_upper = service.calculate_trend_score(content)

        content_lower = {"original_text": "ai machine learning", "title": "ai"}
...
class TestFreshnessDashboard:
    ...
    def test_dashboard_with_no_scores(self, mock_supabase, mock_user):
        ...
        # Mock content count
        mock_client.table.return_value.select.return_value.eq.return_value.execute.return_value.count = (
            10
        )
...
class TestBulkAnalyze:
    ...
    def test_bulk_analyze_with_content_ids(self, mock_supabase, mock_user):
        ...
        result = asyncio.run(
            bulk_analyze_content(content_ids=content_ids, user=mock_user)
        )
...
class TestStaleContent:
    ...
    def test_list_stale_content(self, mock_supabase, mock_user):
        ...
        # Mock count
        mock_client.table.return_value.select.return_value.eq.return_value.lt.return_value.execute.return_value.count = (
            1
        )

        result = asyncio.run(
            list_stale_content(threshold=50, page=1, page_size=20, user=mock_user)
        )
