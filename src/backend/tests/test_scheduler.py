...
class TestSchedulerService:
    ...
    def test_schedule_post_timezone_conversion(self, scheduler_service, mock_supabase):
        ...
        # Mock successful insert
        future_time = datetime.now(pytz.timezone("America/New_York")) + timedelta(hours=1)
        mock_supabase.execute.return_value = MagicMock(data=[{
            "id": "test-post-id",
            "user_id": "test-user-id",
            "content_id": "test-content-id",
            "platform": "twitter",
            "scheduled_at": future_time.astimezone(pytz.UTC).replace(tzinfo=None).isoformat(),
            "status": "pending",
            "asset_type": "post",
            "settings": {},
    ...
    def test_get_scheduled_posts_with_filters(self, scheduler_service, mock_supabase):
        ...
        results = scheduler_service.get_scheduled_posts("test-user-id", status="pending", platform="twitter")
    ...
    def test_update_scheduled_post(self, scheduler_service, mock_supabase):
        ...
        result = scheduler_service.update_scheduled_post("test-user-id", "post-1", update_request)
