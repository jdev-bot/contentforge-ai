...
with patch.dict(
    "sys.modules", {"feedparser": MagicMock(parse=MagicMock(return_value=mock_feed))}
):
...
async def test_create_feed_success(client, auth_headers):
    ...
    with patch("app.routers.rss.rss_service.validate_feed") as mock_validate, patch(
        "app.core.supabase.get_supabase_client"
    ) as mock_supabase, patch("app.tasks.rss.fetch_single_feed_task") as mock_task:
...
async def test_list_feeds_success(client, auth_headers, sample_rss_feed):
    ...
    with patch("app.core.supabase.get_supabase_client") as mock_supabase:
        ...
        mock_table.select.return_value = MagicMock(return_value=mock_query)
        ...
        data_query.order.return_value = MagicMock(
            limit=MagicMock(
                offset=MagicMock(
                    return_value=MagicMock(
                        execute=MagicMock(
                            return_value=MagicMock(data=[sample_rss_feed])
                        )
                    )
                )
            )
        ...
        mock_table.select = MagicMock(
            side_effect=lambda *args, **kwargs: (
                count_query if kwargs.get("count") else data_query
            )
        )
        mock_client.table.return_value = mock_table
        mock_supabase.return_value = mock_client
...
async def test_get_feed_success(client, auth_headers, sample_rss_feed):
    ...
    with patch("app.core.supabase.get_supabase_client") as mock_supabase:
        mock_client = MagicMock()
        mock_query = MagicMock()
        mock_query.single.return_value.execute.return_value = MagicMock(
            data=sample_rss_feed
        )
        ...
        response = client.get(
            f"/api/v1/rss/feeds/{sample_rss_feed['id']}", headers=auth_headers
        )
...
async def test_update_feed_success(client, auth_headers, sample_rss_feed):
    ...
    with patch("app.core.supabase.get_supabase_client") as mock_supabase:
        ...
        mock_select_query.single.return_value.execute.return_value = MagicMock(
            data=sample_rss_feed
        )
...
async def test_manual_fetch_success(client, auth_headers, sample_rss_feed):
    ...
    with patch("app.core.supabase.get_supabase_client") as mock_supabase, patch(
        "app.routers.rss.rss_service.fetch_feed"
    ) as mock_fetch:
        ...
        response = client.post(
            f"/api/v1/rss/feeds/{sample_rss_feed['id']}/fetch", headers=auth_headers
        )
...
async def test_list_entries_success(client, auth_headers, sample_rss_entry):
    ...
    with patch("app.core.supabase.get_supabase_client") as mock_supabase:
        ...
        mock_table = MagicMock()
        mock_table.select.return_value.eq.return_value = MagicMock(
            execute=MagicMock(return_value=mock_feeds_result)
        )
        mock_client.table.return_value = mock_table
        mock_supabase.return_value = mock_client
...
@pytest.mark.asyncio
@pytest.mark.skip(reason="RSS router not properly initialized in test environment")
async def test_import_entry_success(
    client, auth_headers, sample_rss_entry, sample_content
):
    """Test importing an RSS entry as content."""
    with patch("app.core.supabase.get_supabase_client") as mock_supabase, patch(
        "app.routers.rss.rss_service.import_entry"
    ) as mock_import:
        ...
        mock_entry_query.single.return_value.execute.return_value = MagicMock(
            data=sample_rss_entry
        )

        # Mock feed ownership check
        mock_feed_query = MagicMock()
        mock_feed_query.single.return_value.execute.return_value = MagicMock(
        ...
        response = client.post(
            f"/api/v1/rss/entries/{sample_rss_entry['id']}/import", headers=auth_headers
        )
...
@pytest.mark.asyncio
async def test_import_entry_already_processed(
    client, auth_headers, sample_rss_entry, sample_content
):
    ...
    mock_feed_query = MagicMock()
    mock_feed_query.eq.return_value.single.return_value.execute.return_value = (
        MagicMock(data={"user_id": "test-user-id-123"})
    )
...
class TestRSSService:
    ...
    async def test_validate_feed_success(self):
        ...
        with patch("httpx.AsyncClient.get") as mock_get:
            ...
            with patch.object(feedparser, "parse", return_value=mock_parsed):
                is_valid, error = await rss_service.validate_feed(
    ...
    async def test_fetch_feed_success(self, sample_rss_feed):
        ...
        with patch(
        ...
        ) as mock_supabase, patch("httpx.AsyncClient.get") as mock_get:
            ...
            with patch.object(feedparser, "parse", return_value=mock_parsed):
                ...
                result = await rss_service.fetch_feed(
                    sample_rss_feed["id"], sample_rss_feed["user_id"]
                )
...
class TestRSSTasks:
    ...
    def test_process_rss_entry_task(self):
        ...
        with patch("app.tasks.rss.rss_service.import_entry") as mock_import:
            ...
            result = process_rss_entry_task.run(
                "entry-id-123", "user-id-123", "project-id-123"
            )
    ...
    def test_retry_failed_feeds_task(self):
        ...
        with patch(
            ...
            "app.tasks.rss.fetch_single_feed_task"
        ) as mock_fetch_task:
...
class TestRSSErrors:
    ...
    async def test_fetch_feed_network_error(
        self, client, auth_headers, sample_rss_feed
    ):
        ...
        with patch("app.core.supabase.get_supabase_client") as mock_supabase:
            ...
            with patch(
                ...
                response = client.post(
                    f"/api/v1/rss/feeds/{sample_rss_feed['id']}/fetch",
                    headers=auth_headers,
                )
