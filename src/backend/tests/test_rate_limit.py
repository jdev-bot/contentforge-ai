...
class TestCheckAndIncrementUsage:
    ...
    def test_check_usage_under_limit(self):
        ...
        mock_client.table.return_value.select.return_value.eq.return_value.single.return_value.execute.return_value = create_mock_supabase_response(
            data=profile_data
        )
        mock_client.table.return_value.update.return_value.eq.return_value.execute.return_value = create_mock_supabase_response(
            data=[{**profile_data, "monthly_usage_count": 51}]
        )
    ...
    def test_check_usage_at_limit(self):
        ...
        with patch("app.core.rate_limit.get_supabase_client", return_value=mock_client):
            ...
            assert (
                "limit" in str(exc_info.value.detail).lower()
                or "reached" in str(exc_info.value.detail).lower()
            )
...
class TestGetUsageHistory:
    ...
    def test_get_usage_history_success(self):
        ...
        mock_client = MagicMock()
        mock_client.table.return_value.select.return_value.eq.return_value.order.return_value.limit.return_value.execute.return_value = create_mock_supabase_response(
            data=history_data
        )
