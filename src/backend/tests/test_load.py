...
class TestConcurrentUsers:
    ...
    def test_concurrent_registration_10_users(self, client):
        ...
        for i, status_code in results:
            assert status_code == status.HTTP_201_CREATED, f"User {i} failed with status {status_code}"
    ...
    def test_concurrent_login_50_users(self, client):
        ...
        for i, status_code in results:
            assert status_code == status.HTTP_200_OK, f"User {i} failed with status {status_code}"
    ...
    def test_concurrent_content_creation_100_requests(self, client):
        ...
        rate_limited = sum(1 for _, code in results if code == status.HTTP_429_TOO_MANY_REQUESTS)
        
        assert success_count + rate_limited == 100, f"Expected 100 results, got {len(results)}. Errors: {errors}"
        # At least 50% should succeed
        assert success_count >= 50, f"Only {success_count} succeeded, expected at least 50"
    ...
    def test_concurrent_get_requests_100_users(self, client):
        ...
        def get_health(i):
            try:
                ...
                results.append((i, response.status_code, response.elapsed.total_seconds()))
            except Exception as e:
                errors.append((i, str(e)))
