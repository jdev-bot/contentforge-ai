...
from tests.conftest import (
    _build_mock_supabase_client, _build_mock_usage_stats,
    _SUPABASE_CLIENT_PATCH_TARGETS, _ADMIN_CLIENT_PATCH_TARGETS,
    _RATE_LIMIT_FUNC_PATCHES, _safe_patch
)
...
def _make_insert_return(data_dict):
    ...
    row = {**data_dict, "id": str(uuid4()), "created_at": "2026-04-14T10:00:00Z", "updated_at": "2026-04-14T10:00:00Z"}
    result = MagicMock()
    result.data = [row]
    result.count = 1
    return result
...
def _make_table_mock(table_name):
    ...
    select_chain.select = MagicMock(side_effect=lambda *a, **kw: count_chain if (a and a[0] == "count") else select_chain)
    ...
    t.insert.side_effect = lambda *args, **kwargs: _insert_and_return(table_name, *args, **kwargs)
    t.upsert.side_effect = lambda *args, **kwargs: _upsert_and_return(table_name, *args, **kwargs)
...
def _insert_and_return(table_name, data=None, **kwargs):
    ...
    if data:
        if isinstance(data, list):
            for item in data:
                row = {**item, "id": str(uuid4()), "created_at": "2026-04-14T10:00:00Z", "updated_at": "2026-04-14T10:00:00Z"}
                _mock_data_store.setdefault(table_name, []).append(row)
        elif isinstance(data, dict):
            row = {**data, "id": str(uuid4()), "created_at": "2026-04-14T10:00:00Z", "updated_at": "2026-04-14T10:00:00Z"}
            _mock_data_store.setdefault(table_name, []).append(row)
    chain = MagicMock()
    rows = _mock_data_store.get(table_name, [])
    def insert_execute():
...
for t in ["app.routers.auth.send_welcome_email_task", "app.routers.rss.fetch_single_feed_task", "app.routers.stripe.send_invoice_receipt_task"]:
...
_groq_tuple = AsyncMock(return_value=("Mocked AI generated content about the topic.", 150))

for method in ["generate_content", "generate_social_posts", "generate_thread",
               "generate_newsletter", "generate_short_video_script"]:
...
with patch('app.core.rate_limit.UsageTrackingMiddleware', NoOpMiddleware):
    with patch('app.core.error_tracking.ErrorTrackingMiddleware', NoOpMiddleware):
        ...
        from app.core.rate_limit import enforce_subscription_limit, rate_limit_dependency
...
def test(name, method, url, data=None, params=None, expected_min=200, expected_max=299, check_fn=None):
    ...
    try:
        ...
        if status_ok and body_ok:
            ...
            if not status_ok:
                results["errors"][name] = f"Expected {expected_min}-{expected_max}, got {r.status_code}: {detail}"
...
test("health_detailed", "get", "/api/v1/health/detailed", expected_min=200, expected_max=503)

# ── 2. Auth ──────────────────────────────────────────────────────────
test_group("2. Authentication")
test("auth_register", "post", "/api/v1/auth/register", data={
    "email": "systest@contentforge.ai", "password": "TestPass123!", "full_name": "System Test"
})
test("auth_me", "get", "/api/v1/auth/me")
# Login expected to fail with mock auth (no real Supabase verify)
test("auth_login", "post", "/api/v1/auth/login", data={
    "email": "systest@contentforge.ai", "password": "TestPass123!"
}, expected_min=200, expected_max=401)

# ── 3. Organizations ─────────────────────────────────────────────────
test_group("3. Organizations")
test("org_list", "get", "/api/v1/organizations/")
test("org_create", "post", "/api/v1/organizations/", data={
    "name": "Test Org", "slug": "test-org-sys"
})
...
test("project_create", "post", "/api/v1/projects", data={
    "name": "Test Project", "description": "System test"
})
...
test("content_create", "post", "/api/v1/content", data={
    "title": "Test Article", "project_id": PROJ_ID,
    "source": {"type": "text", "text": "Test content body."}
})
test("content_get", "get", f"/api/v1/content/{CONTENT_ID}")
test("content_generate", "post", f"/api/v1/content/{CONTENT_ID}/generate", data={
    "platform": "twitter"
})
...
test("dist_create", "post", "/api/v1/distributions", data={
    "asset_id": UUID1, "platform": "twitter",
...
test("usage_summary", "get", "/api/v1/usage/summary", expected_min=200, expected_max=404)
...
test("ai_improve", "post", "/api/v1/ai-suggestions/improve", data={
    "content_id": CONTENT_ID, "suggestion_type": "readability"
})
test("ai_seo", "post", "/api/v1/ai-suggestions/seo", data={
    "content_id": CONTENT_ID, "keywords": ["AI", "content"], "target_audience": "developers"
})
test("ai_tone", "post", "/api/v1/ai-suggestions/tone", data={
    "content_id": CONTENT_ID, "tone": "professional"
})
test("ai_rewrite", "post", "/api/v1/ai/edit/rewrite", data={
    "content": "Rewrite this.", "instructions": "Make engaging", "tone": "professional", "style": "neutral"
})
test("ai_expand", "post", "/api/v1/ai/edit/expand", data={
    "content": "Expand this.", "instructions": "More detail", "target_length": 2
})
test("ai_condense", "post", "/api/v1/ai/edit/condense", data={
    "content": "Condense this overly long piece of text.", "instructions": "Shorter"
})
test("ai_optimize", "post", "/api/v1/ai/edit/optimize", data={
    "content": "Optimize this.", "platform": "twitter", "tone": "professional"
})
...
test("auto_rules_create", "post", "/api/v1/automation/rules", data={
    "name": "Test Rule", "trigger_type": "content_created",
...
test("schedule_create", "post", "/api/v1/schedule", data={
    "content_id": CONTENT_ID, "platform": "twitter",
    "scheduled_at": "2026-04-15T10:00:00Z"
})
test("schedule_stats", "get", "/api/v1/schedule/stats")
test("schedule_upcoming", "get", "/api/v1/schedule/upcoming")
test("schedule_bulk", "post", "/api/v1/schedule/bulk", data=[
    {"content_id": CONTENT_ID, "platform": "twitter", "scheduled_at": "2026-04-15T10:00:00Z"}
])
...
test("rss_feeds_create", "post", "/api/v1/rss/feeds", data={
    "url": "https://example.com/feed.xml", "name": "Test Feed"
}, expected_min=200, expected_max=499)
...
test("freshness_bulk", "post", "/api/v1/freshness/bulk-analyze", data=[CONTENT_ID, UUID2])
...
test("audience_record", "post", "/api/v1/audience/record", data={
    "platform": "twitter", "metric_type": "followers", "value": 1000, "period": "daily"
})
...
test("trends_track", "post", "/api/v1/trends/track", data={
    "topic_id": UUID1, "relevance_score": 0.85
})
...
test("integration_create", "post", "/api/v1/integrations", data={
    "name": "Test Integration", "integration_type": "webhook",
...
test("alerts_check_metrics", "post", "/api/v1/alerts/check-metrics", data={
    "content_id": CONTENT_ID, "metrics": {"views": 100, "engagement": 5.0}
})
...
test("competitors_create", "post", "/api/v1/competitors", data={
    "name": "Competitor X", "platform": "twitter", "handle": "@competitorx"
})

# ── 21. Version History ──────────────────────────────────────────────
test_group("21. Version History")
test("version_list", "get", f"/api/v1/content/{CONTENT_ID}/versions")
test("version_create", "post", f"/api/v1/content/{CONTENT_ID}/versions", data={
    "body": "New version content", "change_summary": "Updated"
})
test("version_diff", "get", f"/api/v1/content/{CONTENT_ID}/versions/diff", params={"v1": UUID1, "v2": UUID2})
...
test("quality_analyze", "post", "/api/v1/quality/analyze", data={
    "text": "Well-written test article about AI technology and its impact."
})
test("quality_batch", "post", "/api/v1/quality/batch", data={
    "items": [{"content_id": UUID1, "text": "Test content one."}, {"content_id": UUID2, "text": "Test content two."}]
})

# ── 24. Sentiment ────────────────────────────────────────────────────
test_group("24. Sentiment Analysis")
test("sentiment_analyze", "post", "/api/v1/sentiment/analyze", data={
    "text": "We are thrilled to announce our new product!"
})
test("sentiment_batch", "post", "/api/v1/sentiment/batch", data={
    "items": [{"content_id": UUID1, "text": "Great news everyone!"}, {"content_id": UUID2, "text": "This is terrible."}]
})
...
test("dashboard_create", "post", "/api/v1/dashboards", data={
    "name": "Test Dashboard", "description": "System test"
})

# ── 26. Reports ──────────────────────────────────────────────────────
test_group("26. Reports")
test("reports_list", "get", "/api/v1/reports")
test("report_create", "post", "/api/v1/reports", data={
    "name": "Test Report", "report_type": "content_summary",
    "schedule": "0 9 * * *"
})
...
test("retention_create", "post", "/api/v1/retention/policies", data={
    "content_type": "content", "archive_after_days": 180, "delete_after_days": 365
})

# ── 28. Comments v2 ─────────────────────────────────────────────────
test_group("28. Comments v2")
test("comments_list", "get", f"/api/v1/content/{CONTENT_ID}/comments")
test("comment_create", "post", f"/api/v1/content/{CONTENT_ID}/comments", data={
    "text": "Test comment with @mention"
})
test("comment_mentions_lookup", "get", "/api/v1/comments/mentions/lookup", params={"q": "test"})
...
test("categorization_auto_tag", "post", "/api/v1/categorization/auto-tag", data={
    "content_id": CONTENT_ID, "max_tags": 5
})
test("categorization_cluster", "post", "/api/v1/categorization/cluster", data={
    "cluster_count": 3
})
...
test("perf_track", "post", "/api/v1/performance/track", data={
    "content_id": CONTENT_ID, "event_type": "view", "value": 100
})
...
test("sso_create_provider", "post", "/api/v1/sso/providers", data={
    "name": "google", "display_name": "Google", "client_id": "test-id",
...
test("saml_create_provider", "post", "/api/v1/saml/providers", data={
    "name": "okta", "display_name": "Okta",
    "entity_id": "https://okta.com/saml",
    "sso_url": "https://okta.com/sso/saml", "certificate": "test-cert"
})
...
test("plugin_create", "post", "/api/v1/plugins", data={
...
test("sla_create_policy", "post", "/api/v1/sla/policies", data={
    "name": "Test SLA", "metric": "uptime", "threshold": 99.9, "window_minutes": 5
})
...
test("int_framework_create", "post", "/api/v1/integration-framework/configs", data={
    "name": "Test Config", "type": "webhook", "provider": "custom",
    "credentials": {"api_key": "test"}, "settings": {"url": "https://example.com"}
})

# ── 38. Funnel Tracking ─────────────────────────────────────────────
test_group("38. Funnel Tracking")
test("funnels_list", "get", "/api/v1/funnels")
test("funnel_create", "post", "/api/v1/funnels", data={
    "name": "Test Funnel", "steps": [
...
test("attribution_calculate", "post", "/api/v1/attribution/calculate", data={
    "content_id": CONTENT_ID, "model": "last_touch"
})
...
test("stripe_checkout", "post", "/api/v1/stripe/checkout", data={
...
test("admin_error_summary", "get", "/api/v1/admin/errors/summary", expected_min=200, expected_max=403)
test("admin_user_stats", "get", "/api/v1/admin/users/stats", expected_min=200, expected_max=403)
