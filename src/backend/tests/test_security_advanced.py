...
class TestSQLInjection:
    ...
    def test_sql_injection_in_password_field(self, client):
        ...
        for payload in sql_injection_payloads:
            ...
            assert (
                "SQL" not in error_detail.upper()
            ), f"SQL error leaked for payload: {payload[:30]}"
    ...
    def test_sql_injection_in_search_query(self, client):
        ...
        for payload in search_payloads:
            ...
            response = client.get(
                f"/api/v1/content?search={payload}",
                headers={"Authorization": "Bearer test-token"},
            )
...
class TestXSSProtection:
    ...
    def test_xss_in_headers(self, client):
        ...
        for payload in xss_payloads:
            response = client.get(
                "/api/v1/health",
                headers={"X-Custom-Header": payload, "User-Agent": payload},
            )
...
class TestJWTSecurity:
    ...
    def test_malformed_token(self, client):
        ...
        for token in malformed_tokens:
            response = client.get(
                ...
                headers={
                    "Authorization": (
                        f"Bearer {token}" if token.startswith("Bearer") else token
                    )
                },
            )
    ...
    def test_token_injection_via_query_param(self, client):
        ...
        response = client.get(
            "/api/v1/auth/me?token=eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJhZG1pbiJ9.invalid"
        )
    ...
    def test_token_timing_attack_prevention(self, client):
        ...
        if avg_valid > 0:
            ...
            assert (
                diff_ratio < 0.5
            ), f"Timing difference too large: {diff_ratio*100:.1f}%"
    ...
    def test_path_traversal_in_endpoint(self, client):
        ...
        for path in traversal_paths:
            ...
            if response.status_code == status.HTTP_200_OK:
                ...
                assert (
                    "[extensions]" not in body
                ), "Windows path traversal may have succeeded"
...
class TestInputValidation:
    ...
    def test_oversized_payload_rejection(self, client):
        ...
        large_payload = {
            ...
            "source": {"type": "text", "text": "B" * 10000000},  # 10MB text
            "project_id": "123e4567-e89b-12d3-a456-426614174001",
        }
    ...
    def test_version_disclosure(self, client):
        ...
        for header in sensitive_headers:
            if header in headers:
                ...
                assert (
                    "Windows" not in value or "Apache" not in value
...
class TestSessionSecurity:
    ...
    def test_session_fixation_protection(self, client):
        ...
        response = client.post(
            "/api/v1/auth/login",
            json={"email": "victim@example.com", "password": "CorrectPassword123!"},
        )
    ...
    def test_logout_invalidates_token(self, client):
        ...
        response = client.get(
            "/api/v1/auth/me", headers={"Authorization": "Bearer logged-out-token"}
        )
