...
def create_mock_supabase_response(data: Optional[Any] = None, error: Optional[str] = None):
    """
...
def assert_api_error(response, expected_status: int, expected_detail: Optional[str] = None):
    """
        ...
        def table_side_effect(name):
            ...
            if name in self._table_responses:
                ...
                mock_table.insert.return_value.execute.return_value = \
                    create_mock_supabase_response(data=[data] if not isinstance(data, list) else data)
