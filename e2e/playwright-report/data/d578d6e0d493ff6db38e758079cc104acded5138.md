...
```ts
  ...
  21  |     const hasItems = await page.locator('[class*="project"]').first().isVisible().catch(() => false)
  22  |     const hasEmpty = await page.getByText(/no project|create.*project|empty/i).first().isVisible().catch(() => false)
  23  |     expect(hasItems || hasEmpty).toBeTruthy()
  24  |   })
  25  | 
...
> 107 |       expect(await confirmBtn.isVisible().catch(() => false)).toBeTruthy()
  ...
  112 | })
  113 | 
  114 | // Need API_URL for direct API tests
  115 | import { API_URL } from './helpers'
```