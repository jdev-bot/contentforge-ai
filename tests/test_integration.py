...
class TestFullWorkflow:
    ...
    def test_complete_user_workflow(self, client):
        ...
        list_response = client.get("/api/v1/content", headers={
            "Authorization": f"Bearer {token}"
        })
        ...
        get_response = client.get(f"/api/v1/content/{content_id}", headers={
            "Authorization": f"Bearer {token}"
        })
        ...
        assets_response = client.get(f"/api/v1/content/{content_id}/assets", headers={
            "Authorization": f"Bearer {token}"
        })
    ...
    def test_workflow_with_url_source(self, client):
        ...
        with patch("app.services.extraction_service.content_extraction_service") as mock_extract:
...
class TestThirdPartyServiceFailures:
    ...
    def test_youtube_extraction_failure(self, client):
        ...
        with patch("app.services.extraction_service.content_extraction_service") as mock_extract:
            mock_extract.extract_from_youtube.side_effect = Exception("Video not available")
